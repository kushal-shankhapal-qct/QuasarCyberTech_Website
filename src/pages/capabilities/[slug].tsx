...
const renderHighlightedText = (text: string, terms: string[], color: string) => {
  if (!terms.length) {
    return text;
  }
...
const getFocusAreaIcon = (text: string): LucideIcon => {
  ...
  if (value.includes('investment') || value.includes('budget')) return TrendingUp;
  if (value.includes('governance')) return Landmark;
  if (value.includes('decision') || value.includes('readiness')) return Target;
  if (value.includes('third-party') || value.includes('ecosystem') || value.includes('vendor')) return Network;
  if (value.includes('architecture') || value.includes('segmentation')) return LayoutDashboard;
  if (value.includes('identity') || value.includes('authentication')) return Fingerprint;
  if (value.includes('cloud')) return Cloud;
  if (value.includes('policy') || value.includes('compliance')) return FileCheck;
...
const getDeliveryStepWatermarkIcon = (text: string): LucideIcon => {
  ...
  if (value.includes('response') || value.includes('alignment')) return ShieldCheck;
  if (value.includes('reporting') || value.includes('roadmap') || value.includes('delivery')) return BarChart3;
...
const getIndustrySlug = (name: string) => {
  ...
  if (name.includes('E-commerce') || name.includes('Digital')) return 'ecommerce';
  if (name.includes('Healthcare') || name.includes('HealthTech')) return 'healthcare';
  if (name.includes('Enterprise') || name.includes('Manufacturing')) return 'enterprise';
...
const getPlatformLogo = (logoKey: string) => {
  ...
  if (logoKey === 'qstellarDark') {
    return ASSETS.logos.platforms.qstellarLight;
  }
...
const PLATFORM_VISUAL_CONTROLS = {
  cardBackground: 'linear-gradient(135deg, rgba(107,21,48,0.94) 0%, rgba(55,24,72,0.93) 44%, rgba(11,31,59,0.95) 100%)',
...
const CapabilityPage: React.FC = () => {
  ...
  const [tabBarMode, setTabBarMode] = useState<'relative' | 'fixed'>('relative');
  const [dockedTop, setDockedTop] = useState<number>(Number.parseInt(NAVBAR_HEIGHT, 10) || 120);
  ...
  const activeSub: SubCapability | null = useMemo(() => {
    ...
    return capability.subCapabilities.find((subCapability) => subCapability.slug === activeSubSlug) || capability.subCapabilities[0] || null;
  ...
  useEffect(() => {
    ...
    const sources = capability.subCapabilities
      .map((subCapability) => subCapability.image || subCapabilityImages[subCapability.slug])
  ...
  useEffect(() => {
    ...
    const getNavbarBottom = () => {
      const cssVarBottom = window
        ...
        .trim();
      const parsedCssVarBottom = Number.parseFloat(cssVarBottom.replace('px', ''));
      if (!Number.isNaN(parsedCssVarBottom)) {
        return parsedCssVarBottom;
      }
    ...
    const updateStickyState = () => {
      if (!capabilityContentRef.current || !tabBarRef.current) {
        return;
      }

      const navbarBottom = getNavbarBottom();
      const navbarOffset = Math.max(0, (navbarBottom ?? fallbackNavbarOffset) + TABBAR_NAV_GAP_PX);

      const contentTop = capabilityContentRef.current.getBoundingClientRect().top + window.scrollY;
      const contentBottom = capabilityContentRef.current.getBoundingClientRect().bottom + window.scrollY;
      ...
      setTabBarHeight((prev) => (prev === measuredTabHeight ? prev : measuredTabHeight));

      const scrollY = window.scrollY;
      const fixStart = contentTop - navbarOffset;
      let nextMode: 'relative' | 'fixed' = 'relative';
      if (scrollY >= fixStart) {
        nextMode = 'fixed';
      }
      ...
      const nextDockedTop = Math.min(
        navbarOffset,
        stopBoundary - scrollY,
      );
  ...
  const scrollToSubCapabilityTop = () => {
    if (!subCapabilityStartRef.current) {
      return;
    }
    ...
    const cssVarBottom = window
      ...
      .trim();
    const parsedCssVarBottom = Number.parseFloat(cssVarBottom.replace('px', ''));
    const navbarOffset = Number.isNaN(parsedCssVarBottom) ? fallbackNavbarOffset : Math.max(0, parsedCssVarBottom);
    const tabOffset = tabBarRef.current?.offsetHeight || 0;
    const targetTop = subCapabilityStartRef.current.getBoundingClientRect().top + window.scrollY - navbarOffset - tabOffset - 8;
  ...
  const activeVisual = activeSub.image || subCapabilityImages[activeSub.slug] || '';
  const platform = capability.platform ? platformConfigs[capability.platform] : null;
  const activePlatformKey = platform?.id || 'default';
  const platformLogoHeight = PLATFORM_VISUAL_CONTROLS.logoHeight[activePlatformKey as keyof typeof PLATFORM_VISUAL_CONTROLS.logoHeight] || PLATFORM_VISUAL_CONTROLS.logoHeight.default;
  const platformScreenshotHeight = PLATFORM_VISUAL_CONTROLS.screenshotHeight[activePlatformKey as keyof typeof PLATFORM_VISUAL_CONTROLS.screenshotHeight] || PLATFORM_VISUAL_CONTROLS.screenshotHeight.default;
  ...
  return (
    <div className="min-h-screen w-full bg-[#000000]">
      ...
      <PageHero
        title={renderHighlightedText(capability.name, capabilityTitleHighlights, COLORS.gold)}
        highlight=""
      ...
      >
        <div 
          ...
          style={{ 
            ...
            boxShadow: tabBarMode === 'fixed' ? 'none' : '0 2px 8px rgba(0,0,0,0.08)',
            borderBottom: '1px solid #6B1530',
            willChange: tabBarMode === 'fixed' ? 'top' : undefined,
            transition:
              tabBarMode === 'fixed'
                ? TABBAR_NAV_TRANSITION
                : undefined,
          }}
        >
          <div className="max-w-7xl mx-auto grid gap-0" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', minHeight: '52px', paddingLeft: LAYOUT_CONTROLS.section.paddingX, paddingRight: LAYOUT_CONTROLS.section.paddingX }}>
            {capability.subCapabilities.map((subCapability) => {
              ...
              return (
                <button
                  ...
                  onMouseEnter={(e) => {
                    if (!isActive) {
                      e.currentTarget.style.background = 'rgba(107,21,48,0.14)';
        ...
        {tabBarMode !== 'relative' && <div style={{ height: `${tabBarHeight}px` }} />}
        <section ref={subCapabilityStartRef} className="w-full bg-[#FFFFFF] pb-20">
          <div
          ...
          <div className="text-left h-full">
            <div className="mb-6">
              ...
              >
                {renderHighlightedText(activeSub.name, subCapabilityTitleHighlights[activeSub.slug] || [], COLORS.burgundy)}
              </h2>
            </div>
            <p
            ...
            <h3 className="text-black text-lg font-bold mb-3" style={{ fontFamily: TYPOGRAPHY.fontHeading }}>
              What We Assess
            </h3>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-4" style={{ gridAutoRows: '1fr' }}>
                ...
                return (
                  <div key={item} className="group px-2 py-2 h-full">
                    <div className="flex items-center gap-3 h-full">
                      ...
                      <p className="text-sm font-medium text-[#0B1F3B] leading-snug m-0" style={{ fontFamily: TYPOGRAPHY.fontBody }}>
                        {item}
                      </p>
                    </div>
                  </div>
      ...
      <section className="w-full py-20 border-t border-white/10" style={{ background: GRADIENTS.HERO_BG }}>
        <div
          className="max-w-7xl mx-auto text-left"
          style={{ paddingLeft: LAYOUT_CONTROLS.section.paddingX, paddingRight: LAYOUT_CONTROLS.section.paddingX }}
        >
          <h3 className="text-4xl font-bold text-white mb-2" style={{ fontFamily: TYPOGRAPHY.fontHeading }}>
            How <span style={{ color: '#D6B05C' }}>QuasarCyberTech</span> Delivers
          </h3>
          <p className="text-white/70 text-base mb-10" style={{ fontFamily: TYPOGRAPHY.fontBody }}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {(activeSub.approach || []).map((step, index) => {
              ...
              const WatermarkIcon = getDeliveryStepWatermarkIcon(parsed.phase);
              return (
                <div key={`${parsed.phase}-${index}`} className="relative">
                  <div className="relative overflow-hidden border-l-2 border-[#6B1530] pl-3 pr-2 py-1">
                    ...
                    <p className="text-xs font-mono text-[#D6B05C] m-0">{String(index + 1).padStart(2, '0')}</p>
                    <p className="text-sm font-semibold text-white mt-1 mb-1" style={{ fontFamily: TYPOGRAPHY.fontHeading }}>
                      {parsed.phase}
                    </p>
                    <p className="text-xs text-white/65 leading-relaxed m-0" style={{ fontFamily: TYPOGRAPHY.fontBody }}>
                      {parsed.description}
                    </p>
                  </div>
                </div>
      ...
      <section className="w-full bg-[#F5F7FA] border-t border-gray-200" style={{ paddingTop: HS_CAP.sectionPaddingY, paddingBottom: HS_CAP.sectionPaddingY }}>
        <div
        ...
        >
          ...
          >
            <span style={{ color: COLORS.burgundy }}>Industry</span> Application
          </h3>
                ...
                >
                  <div
                    style={{
                      ...
                      background: isHovered ? INDUSTRY_CARD_VISUAL_CONTROLS.accentHover : INDUSTRY_CARD_VISUAL_CONTROLS.accentDefault,
                  ...
                  <div className="h-40 overflow-hidden relative">
                    ...
                    <div className="absolute inset-0 pointer-events-none" style={{ background: 'linear-gradient(160deg, rgba(107,15,43,0.1) 0%, rgba(11,31,59,0.2) 100%)' }} />
                  </div>
                  <div className="p-4">
                    <div className="flex items-center justify-between mb-1 gap-2">
                      <h4 className="font-semibold text-sm text-[#0B1F3B] m-0" style={{ fontFamily: TYPOGRAPHY.fontHeading }}>
                        {industry.name}
                      </h4>
                      <ArrowRight
                        className="w-4 h-4 transition-transform group-hover:translate-x-1"
                        style={{ color: isHovered ? INDUSTRY_CARD_VISUAL_CONTROLS.accentHover : INDUSTRY_CARD_VISUAL_CONTROLS.accentDefault }}
                      />
                    </div>
                    <p className="text-xs text-[#4A5568] leading-relaxed m-0" style={{ fontFamily: TYPOGRAPHY.fontBody }}>
          ...
          {relatedCapabilities.length > 0 && (
            <div style={{ marginTop: HS_CAP.relatedMarginTop, paddingTop: HS_CAP.relatedPaddingTop }}>
              <h3
                className="text-left"
                style={{
                  fontFamily: TYPOGRAPHY.fontHeading,
              ...
              >
                Related <span style={{ color: COLORS.burgundy }}>Capabilities</span>
              </h3>
              <p className="text-[#4A5568] text-sm mb-8 max-w-3xl" style={{ fontFamily: TYPOGRAPHY.fontBody }}>
      ...
      {platform && (
        <section ref={stopStickyRef} className="w-full py-14 border-t border-white/10" style={{ background: GRADIENTS.HERO_BG }}>
          <div
            className="max-w-7xl mx-auto"
            style={{ paddingLeft: LAYOUT_CONTROLS.section.paddingX, paddingRight: LAYOUT_CONTROLS.section.paddingX }}
          >
            <h3 className="text-4xl font-bold text-white mb-2" style={{ fontFamily: TYPOGRAPHY.fontHeading }}>
              Powered by the <span style={{ color: '#D6B05C' }}>QuasarCyberTech</span> ecosystem
            </h3>
            <p className="text-white/70 text-base mb-10" style={{ fontFamily: TYPOGRAPHY.fontBody }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
              <div>
                ...
                />
                <h3 className="text-white text-xl font-semibold mb-2" style={{ fontFamily: TYPOGRAPHY.fontHeading }}>
                  {platform.description}
                </h3>
                <p className="text-sm text-white/60 mb-6" style={{ fontFamily: TYPOGRAPHY.fontBody }}>
                  Our {capability.navLabel} engagements are accelerated by {platform.name}, combining platform intelligence with advisory and execution delivery.
                </p>
                <ul className="space-y-2 mb-6 list-none p-0 m-0">
                  {platform.highlights.slice(0, 3).map((item) => (
                    <li key={item} className="flex items-center gap-2 text-sm text-white/80" style={{ fontFamily: TYPOGRAPHY.fontBody }}>
      ...
      {(capability.faqs || []).length > 0 && (
        <section className="w-full bg-[#FFFFFF] py-16 border-t border-gray-200">
          ...
          >
            ...
            >
              Frequently Asked <span style={{ color: COLORS.burgundy }}>Questions</span>
            </h3>
            <p className="text-[#4A5568] text-sm mb-8" style={{ fontFamily: TYPOGRAPHY.fontBody }}>
            ...
            <div className="flex flex-col gap-3">
              {(capability.faqs || []).slice(0, 5).map((faq, index) => {
                ...
                return (
                  <article key={itemId} className="border border-black/10 bg-white" style={{ borderRadius: '0 0 10px 10px' }}>
                    <button
                      type="button"
                      aria-expanded={isOpen}
                      onClick={() => setExpandedFaq((prev) => (prev === itemId ? null : itemId))}
                      className="w-full text-left px-5 py-4 bg-transparent border-0 cursor-pointer flex items-start justify-between gap-4"
                    >
                      <span className="text-[#0B1F3B] text-base font-semibold" style={{ fontFamily: TYPOGRAPHY.fontHeading }}>
                        {faq.question}
                      </span>
                      <span className="text-[#6B1530] text-lg leading-none" aria-hidden>
                        {isOpen ? '−' : '+'}
                      </span>
                    </button>
                    {isOpen && (
                      <div className="px-5 pb-5">
                        <p className="text-sm text-[#4A5568] leading-relaxed m-0" style={{ fontFamily: TYPOGRAPHY.fontBody }}>
                          {faq.answer}
                        </p>
                      </div>
                    )}
