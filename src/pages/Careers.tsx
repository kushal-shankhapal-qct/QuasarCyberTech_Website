        ...
        <section className="max-w-7xl mx-auto px-6 mb-24 text-center">
          ...
          >
            <h1 className="text-5xl md:text-7xl font-bold text-[#0F172A] mb-6 tracking-tight">
              Defend The <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#8B1E3F] to-[#C41E5E]">Future</span>
            </h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
              {perks.map((p, i) => (
                <div key={i} className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
                  <p.icon className="w-12 h-12 text-[#8B1E3F] mx-auto mb-6" />
          ...
          <div className="space-y-4">
            {jobs.map((job, idx) => (
              <div key={idx} className="flex flex-col md:flex-row items-center justify-between p-6 bg-white border border-gray-200 rounded-xl hover:shadow-lg transition-all duration-300 group cursor-pointer">
                <div>
                  <h3 className="text-xl font-bold text-[#0F172A] group-hover:text-[#8B1E3F] transition-colors">{job.title}</h3>
                ...
                <button className="mt-4 md:mt-0 px-6 py-2 bg-gray-100 text-[#0F172A] font-bold rounded-lg group-hover:bg-[#8B1E3F] group-hover:text-white transition-colors">
                  Apply Now
                </button>
              </div>
            ))}
